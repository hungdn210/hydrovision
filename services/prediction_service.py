...
class PredictionService:
    ...
    def _diagnostics_summary(
        ...
        fallback_html = (
            f'<p><strong>Residual Diagnostics</strong></p>'
            f'<p>Residual checks for <strong>{feature.replace("_", " ")}</strong> were computed from the <strong>{diagnostics_source}</strong>. '
            f'The model shows a {bias_desc}. Residual standard deviation is {res_std:.3f} {unit}, and the corresponding 95% confidence band is approximately ±{conf_bound:.3f} {unit}. '
            f'{spread_note} Overall, the residual pattern suggests the model is <strong>{overall}</strong>. {overall_note}</p>'
            f'<p><strong>Autocorrelation Structure</strong></p>'
            f'<ul>'
            f'<li><strong>ACF:</strong> {acf_desc}{acf_severity}</li>'
            f'<li><strong>PACF:</strong> {pacf_desc}</li>'
            f'<li><strong>Interpretation:</strong> Residual independence matters because remaining serial structure means the forecast model has not fully absorbed the temporal dynamics present in the observations.</li>'
            f'</ul>'
            f'<p><strong>Model Adequacy</strong></p>'
            f'<p>When residuals approximate white noise, forecast errors are more likely to be random and uncertainty estimates are easier to trust. '
            f'When significant ACF or PACF spikes remain, the model may still be missing persistence, lagged response, or seasonal structure that could bias forecast confidence. '
            f'In practice, these diagnostics should be read together with the backtest accuracy metrics before relying on the forecast for operational decisions.</p>'
        )
        ...
        try:
            prompt = (
                ...
                'RESPONSE FORMAT (STRICT — markdown only):\n\n'
                '**Residual Diagnostics**\n'
                'Write 4-5 sentences covering bias, residual spread, whether the confidence band is tight or wide, and the main conclusion on forecast reliability.\n\n'
                '**Autocorrelation Structure**\n'
                'Write 3 bullet points covering ACF behaviour, PACF behaviour, and what the significant lags imply for model refinement.\n\n'
                '**Model Adequacy**\n'
                'Write 3-4 sentences on whether residuals resemble white noise, whether the model is well specified, and what practical caution a forecaster should apply.\n\n'
                'RULES: Cite numbers in every section. No intro or sign-off.\n\n'
    ...
    def _historical_summary(
        ...
        fallback_html = (
            f'<p><strong>Historical Fit Assessment</strong></p>'
            f'<p>Backtest performance for <strong>{feature.replace("_", " ")}</strong> at <strong>{station.replace("_", " ")}</strong> '
            f'is rated <strong>{acc_label}</strong> based on RMSE = {rmse_str}{rmse_pct_of_mean} and MAPE = {mape_str}. '
            f'The historical mean of the observed series is {hist_mean:.3f} {unit}, which provides the baseline against which model error should be judged. '
            f'Lower RMSE matters most where absolute forecast magnitude drives risk, while MAPE helps assess relative skill across stations or features with different scales.</p>'
            f'<p><strong>Evaluation Window and Horizon</strong></p>'
            f'<p>{horizon_note} {coverage_note} '
            f'Because the evaluation spans {n_windows} rolling windows from {date_start} to {date_end}, the reported accuracy is based on repeated historical tests rather than a single split, which generally gives a more credible estimate of operational skill.</p>'
            f'<p><strong>Operational Recommendation</strong></p>'
            f'<ul>'
            f'<li><strong>Decision use:</strong> {recommendation}</li>'
            f'<li><strong>Main caveat:</strong> Historical fit quality can degrade under conditions outside the backtest range, including extreme events, abrupt management interventions, or structural regime shifts.</li>'
            f'<li><strong>Follow-up check:</strong> Compare upcoming forecasts against the newest observations and review residual diagnostics before using the model as a primary decision input.</li>'
            f'</ul>'
        )
        ...
        try:
            prompt = (
                ...
                'Write 4-5 sentences interpreting RMSE, MAPE, accuracy grade, and whether the model fit is operationally strong, moderate, poor, or indeterminate.\n\n'
                '**Evaluation Window and Horizon**\n'
                'Write 3-4 sentences explaining what H means in practice, how many rolling windows were evaluated, and why the date coverage strengthens or limits reliability.\n\n'
                '**Operational Recommendation**\n'
                'Write 3 bullet points covering whether to trust the model for decisions, the main caveat, and one recommended follow-up validation step.\n\n'
                'RULES: Cite numbers. No intro or sign-off.\n\n'
